...
class ExistingCustomerSearchScreen extends StatefulWidget {
  const ExistingCustomerSearchScreen({super.key});
...
class _ExistingCustomerSearchScreenState extends State<ExistingCustomerSearchScreen> {
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                ...
                TextFormField(controller: emailController, decoration: const InputDecoration(labelText: 'Correo')),
  ...
  void _updateCustomer(String idCliente, String nombre, String apellido, String telefono, String correo, String factura) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cliente actualizado con éxito')));
        // ignore: use_build_context_synchronously
        Navigator.of(context).pop(); // Cierra el diálogo de edición
      } else {
        // ignore: use_build_context_synchronously
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error al actualizar el cliente')));
      }
    } catch (e) {
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error durante la actualización')));
    }
  }
  ...
  void _selectClient(String idCliente) async {
    await _storage.write(key: 'selected_client_id', value: idCliente);
    // ignore: use_build_context_synchronously
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cliente seleccionado guardado')));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Buscar Cliente Existente'),
      ),
      body: Padding(
